...
(function () {
  ...
  /* --------------------------------------------------------------------------
     16. HERO VIDEO — sound toggle
     -------------------------------------------------------------------------- */
  const heroVideo = document.querySelector('.hero__video');
  const soundBtn = document.querySelector('.hero__sound-btn');

  if (heroVideo && soundBtn) {
    const iconOn = soundBtn.querySelector('[data-sound-on]');
    const iconOff = soundBtn.querySelector('[data-sound-off]');
    const sync = () => {
      if (iconOn) iconOn.style.display = heroVideo.muted ? 'none' : 'block';
      if (iconOff) iconOff.style.display = heroVideo.muted ? 'block' : 'none';
    };
    sync();
